...
- **Checkpoint latency** — time from `/cancel` to the worker raising `JobCancelledError` inside `annotate`. Bounded by one batch of YOLO inference in pass 1 (`VIDEO_BATCH_SIZE` frames, 8 on NVIDIA by default) or one decoded frame in pass 2. Typically sub-second on GPU; a few seconds on CPU / large models.
