...
2. A grid frame every `max_gap` seconds, or `min_interval` if that is larger (`reason: grid`), counted from the previous grid frame — a motion peak never shifts the grid. The grid has priority over step 3: once it fills `max_frames`, no budget is left for peaks, so a video longer than `(max_frames − 1)` grid steps (≈20 s at the defaults) yields grid frames only, however strong its motion. Past `max_frames` grid steps (≈24 s at the defaults) the grid itself is thinned uniformly, keeping the first and the last grid frame, and the peak step is skipped outright.
