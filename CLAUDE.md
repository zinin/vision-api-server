...
**Motion Frames**: Two ffmpeg passes. Pass 1 streams gray 640 px frames through a pipe and computes `blob`, the area of the largest changed region between neighbouring frames; pass 2 fetches only the selected frames with `select`. Selection: frame 0, a grid every `max_gap` (4 s, or `min_interval` when that is larger), then the strongest motion peaks above `motion_threshold` (0.001) at least `min_interval` (1 s) apart, capped at `max_frames` (6). A segment whose median `blob` exceeds 0.02 (rain, snow in IR) gets the grid only. Real pts from `showinfo`; `video_duration` from ffprobe. Tests use lavfi-generated clips, no binary fixtures.
