...
3. **Прореживание.** Последовательность `S` = первый кадр плюс сетка, длина `L`. Если `L > max_frames = M`: при `M = 1` остаётся только кадр 0; иначе остаются позиции `round_half_up(k × (L − 1) / (M − 1))` для `k = 0 … M − 1`. Первый и последний элементы `S` сохраняются. После прореживания бюджета на пики нет, шаги 4–5 пропускаются.
4. **Шторм.** Если `n ≥ 2` и медиана `blob[1:]` больше `STORM_MEDIAN_BLOB`, отбор заканчивается сеткой.
